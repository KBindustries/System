...
import AnnouncementConfirmDelete from '../components/Announcements/AnnouncementConfirmDelete'
...
function AnnouncementTable() {
    ...
    const filteredData = announcement?.filter(
        (item) =>
    ...
    return (
        <>
            ...
            <div className='flex items-center justify-between'>
                ...
                <Button size="small" onClick={openModal}>Add Announcement</Button>
            </div>
            <AnnouncementModal openModal={openModal} openEditModal={openEditModal} getSpecialties={getSpecialties} getAnnouncement={getAnnouncement} closeModal={closeModal} isModalOpen={isModalOpen} />
                ...
                <Table>
                    ...
                    <TableBody>
                        {filteredData.map((announcement, i) => (
                            <TableRow key={i}>
                                <TableCell>
                                    <span className="text-xs"> {announcement.title}</span>
                                </TableCell>
                                <TableCell>
                                    <span className="text-xs"> {announcement.content}</span>
                                </TableCell>
                                <TableCell>
                                    <span className="text-xs"> {announcement.date.split('T')[0]} </span>
                                </TableCell>
                                {/* split(announcement.date) */}
                                <TableCell>
                                    <span className="text-xs"> {announcement.specialty?.name} </span>
                                </TableCell>
                                <TableCell>
                                    <span className="text-sm">
                                        <button id={announcement._id} onClick={() => openEditModal(announcement._id)} className="hover:text-primary mr-2">
                                            <EditIcon className="w-4 h-4" />
                                        </button>
                                        <button id={announcement._id} onClick={() => openDeleteModal(announcement._id)} className="hover:text-primary">
                                            <svg

...
import CourseConfirmDelete from '../components/Courses/CourseConfirmDelete'
...
function CourseTable() {
    ...
    const [specialties, setGetSpecialties] = useState([])
    ...
    const fetchSpecialties = async () => {
        try {
          ...
          setGetSpecialties(specialtiesData);
        } catch (error) {
    ...
    return (
        <div className='mb-20'>
            ...
            <div className='flex items-center justify-between'>
                ...
                <Button size="small" onClick={openModal}>Add Course</Button>
            </div>
            <CourseModal openModal={openModal} openEditModal={openEditModal} fetchCourses={fetchCourses} specialties={specialties} getCourse={getCourse} closeModal={closeModal} isModalOpen={isModalOpen} />

            <CourseConfirmDelete openModal={openDeleteModal} closeModal={closeDeleteModal} fetchCourses={fetchCourses}  specialties={specialties} getCourse={getCourse} isModalOpen={isDelModalOpen} />
                    ...
                    <TableBody>
                        {filteredData.map((course, i) => (
                            <TableRow key={i}>
                                ...
                                <TableCell>
                                    <span className="text-xs"> {course.specialty_id.name}</span>
                                </TableCell>
                                <TableCell>
                                    <span className="text-xs"> {course.specialty_id.level} </span>
                                </TableCell>
                                <TableCell>
                                    <span className="text-sm">
                                        <button id={course._id} onClick={() => openEditModal(course._id)} className="hover:text-primary mr-2">
                                            <EditIcon className="w-4 h-4" />
                                        </button>
                                        <button id={course._id} onClick={() => openDeleteModal(course._id)} className="hover:text-primary">
                                            <svg
